...
// JSAR Runtime compatibility: Provide fallback webglTestHarness implementation
let webglTestHarness = null;
try {
  webglTestHarness = window.parent && window.parent.webglTestHarness;
} catch (e) {
  // In JSAR Runtime, window.parent might not be accessible
  console.log('webglTestHarness: window.parent not accessible, using fallback');
}

// If webglTestHarness is not available, create a minimal compatible implementation
if (!webglTestHarness) {
  console.log('webglTestHarness: Creating fallback implementation for JSAR Runtime');
  webglTestHarness = {
    reportResults: function(pathname, success, msg, skipped) {
      const status = skipped ? 'SKIPPED' : (success ? 'PASS' : 'FAIL');
      console.log(`[WebGL Test] ${status}: ${pathname} - ${msg || ''}`);
    },
    notifyFinished: function(pathname) {
      console.log(`[WebGL Test] FINISHED: ${pathname}`);
    }
  };
}
